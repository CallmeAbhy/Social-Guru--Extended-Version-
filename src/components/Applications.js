...
const Applications = () => {
  ...
  useEffect(() => {
    axios.get(`http://100.25.193.158:4000/client/find/?email=${val}`)
      .then(response => {
...
console.log(fair);
  ...
  const fetchApiData = async (Id) => {
    try {
      const res = await fetch( `http://100.25.193.158:4000/client/postedJobs?id=${Id}`);
      console.log(res);
      ...
      <Navbar >
        <Container>
          ...
          <Navbar.Collapse className="justify-content-end">
            <Navbar.Text>
              <Button variant='dark' style={{color: 'white'}} as={Link} to="/Posting">Post Job</Button>
            </Navbar.Text>
          </Navbar.Collapse>
        </Container>
      </Navbar>
